...
import {
    Fn, vec2, vec3, vec4, float, sin, cos, fract, floor,
    mix, smoothstep, positionLocal, uniform, dot, clamp, abs, pow, color
} from 'three/tsl'
...
export function Sky({
    ...
    }, [speed, colorCycleSpeed, patternScale, warpIntensity, noiseDetail,
        hueOffset, hueRange, colorBands, colorSharpness, saturation,
        brightness, contrast, shadowIntensity, triplanarSharpness])
    ...
    const skyColorNode = useMemo(() => {
        ...
        const skyShader = Fn(() => {
            ...
            const warpedXY = uvXY.add(globalWarp).add(vec2(
                vnoise(uvXY.add(time.mul(0.06))),
                vnoise(uvXY.add(vec2(3.1, 7.2)).add(time.mul(0.05)))
            ).mul(warpStr.mul(0.4)))

            const warpedXZ = uvXZ.add(globalWarp).add(vec2(
                vnoise(uvXZ.add(time.mul(0.06))),
                vnoise(uvXZ.add(vec2(3.1, 7.2)).add(time.mul(0.05)))
            ).mul(warpStr.mul(0.4)))

            const warpedYZ = uvYZ.add(globalWarp).add(vec2(
                vnoise(uvYZ.add(time.mul(0.06))),
                vnoise(uvYZ.add(vec2(3.1, 7.2)).add(time.mul(0.05)))
            ).mul(warpStr.mul(0.4)))
            ...
            const hue = fract(
                mix(baseHue, nextHue, edgeBlend)
                    .mul(u.hueRange)
                    .add(u.hueOffset)
                    .add(colorTime.mul(0.05))
            )
